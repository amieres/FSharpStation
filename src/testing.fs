    ...
    open WebSharper
        ...
        module TestingJS =
            ...
            //        [<Inline>] static member (<&)(vf:Val<seq<Doc>->'b> , aF:Fun<_,Doc>) = ff vf (VConst (seq [callDoc aF ]) )
                ...
                let varRefToVarD = Depend.depend {
                    ...
                    return fun (VarRef ref) ->
                        match itemRefToTextType ref with
                        | Extract0.TSimple    r
                        | Extract0.TReference r ->
                        let pName, oName = AF.splitName (UoM.Untag currentPlugInName) r
                        AF.tryGetVar pName oName
                        |> Option.map (fun v -> v.varVar)
                        |> Option.defaultWith(fun () -> failwithf "Could not find var %s" r)
                }
                ...
                let parseNewLayout lytN =
                    ...
                    >> refreshEntries (UoM.Untag<_> lytN)
            ...
            module SnippetsUI =
                ...
                let parseNewLY () =
                    currentSnippetContentV.View
                    |> View.Get (NewLY.parseNewLayout <| UoM.Tag<_> "lytTarget2")
