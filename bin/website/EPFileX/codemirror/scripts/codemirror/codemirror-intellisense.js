var Intellisense = function (editor, userCallback, methodsCallback)
{
    ...
    function insertAutoComplete()
    {
        if (decls.isVisible())
        {
            ...
            // Find the end of the current token. If we're inside backtick, try to find end of backtick
            // otherwise, find end of a token (whitespace or things like that)
            var endCh = cursor.ch;
            if (line[autoCompleteStart.columnIndex] == '`' && line[autoCompleteStart.columnIndex + 1] == '`') {
                while (endCh < line.length && line[endCh] != '`') endCh++;
                if (line[endCh] == '`') endCh++;
                if (line[endCh] == '`') endCh++;
            }
            else {
                while (endCh < line.length && line[endCh].match(/[a-zA-Z0-9]/) != null) endCh++;
            }

            var startRange = { line: cursor.line, ch: autoCompleteStart.columnIndex };
            var endRange = { line: cursor.line, ch: endCh };
            editor.replaceRange(name, startRange, endRange);
            editor.setSelection({ line: cursor.line, ch: autoCompleteStart.columnIndex + name.length});
    ...
    function setDeclarations(data)
    {
        ...
        // Filter the declaratons based on the current word
        var cursor = editor.getCursor();
    ...
    editor.on('keydown', function (cm, evt)
    {
        ...
        else if (decls.isVisible())
        {
            ...
            else if (evt.shiftKey && evt.keyCode === 57)
            {
                // left paren, right after we finished typing name
                decls.setVisible(false);
                autoCompleteMethods();
            }
