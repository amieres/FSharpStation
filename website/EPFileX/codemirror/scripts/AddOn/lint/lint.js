...
(function(mod) {
  ...
  function showTooltip(e, content, passOptions) {  // AMG: Added passOptions
	...
	(passOptions.container || document.body).appendChild(tt);   // AMG: Added container field to options
  ...
  function makeMarker(labels, severity, multiple, tooltips, passOptions) {
    ...
    if (tooltips != false) CodeMirror.on(inner, "mouseover", function(e) {
		showTooltipFor(e, labels, inner, passOptions);
    });
  ...
  function lintAsync(cm, getAnnotations, passOptions) {
    ...
    getAnnotations(cm.getValue(), function(annotations, arg2) {
	  ...
	  updateLinting(cm, annotations, passOptions)
    }, passOptions, cm);
  }
  ...
  function startLinting(cm) {
    ...
    if (options.async || getAnnotations.async) {
      ...
      if (annotations.then) annotations.then(function(issues) {
		  updateLinting(cm, issues, passOptions);
      });
	  else updateLinting(cm, annotations, passOptions);
    }
  }

  function updateLinting(cm, annotationsNotSorted, passOptions) {
    clearMarks(cm);
    var state = cm.state.lint, options = state.options;
    ...
    for (var line = 0; line < annotations.length; ++line) {
      ...
      if (state.hasGutter)
        cm.setGutterMarker(line, GUTTER_ID, makeMarker(tipLabel, maxSeverity, anns.length > 1,
			state.options.tooltips, passOptions));
    }
