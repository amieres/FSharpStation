...
#nowarn "1182"
#nowarn "1178"
////-d:FSharpStation1558102682892 -d:NOFRAMEWORK --noframework -d:WEBSHARPER
...
//#define FSharpStation1558102682892
#if INTERACTIVE
module FsRoot   =
#else
namespace FsRoot
    ...
    module Prozper =
        ...
        module Ambiente =
            ...
            type IAmbiente = 
                interface
                    ...
                    abstract member GuardarTransaccion   : IdAliado -> Transaccion ->                 AsyncResultM<unit, unit>
            ...
            let mutable ambiente : IAmbiente = 
                { new IAmbiente with 
                    ...
                    member __.Prepare                   () = ()
                    member __.GuardarTransaccion ida tr    = AsyncResultM.errorMsgf "Ambiente.GuardarTransaccion: not implemented"
                }
        ...
        module Correo =
            ...
            let host = lazy (ambiente.VariableAmbiente "Website" |> sprintf "https://%s")
            ...
            let enviarCorreoInvitacion tema (IdAliado idPadre) (email:string)= 
                TemplateCorreo.Invitacion() 
                    ...
                    .Enlace( sprintf "%s/Register/%s" host.Value idPadre )
                    .Doc()
                |> prepareHtml
            ...
            let enviarBienvenida (aliado:Aliado) (correo:CorreoElectronico) =
                asyncResultM {
                    ...
                    do! TemplateCorreo.Bienvenida()
                            .Logo(   TemplateCorreo.Logo().Doc())
                            .Enlace(        sprintf "%s/#/Content/ProzperLyt.cntFormaFormasPago" host.Value )
                            .Sufijo(        sufijo                                                    )
        ...
        module ObtenerEstado =
            ...
            let obtenerAliadoEstado : AA<_,_> =
                obtenerUsuario
                >>> obtenerAliadoEstado0
        
            let obtenerAliadoEstadoId : AA<_,_> =
                fun (IdAliado id) -> id
                >>> obtenerAliadoEstado0
        ...
        module ManejadorEventos =
            ...
            let obtenerInfluyente : AA<_, _> =
                ObtenerEstado.pairEstado
                >-> fun (es, inf:string)-> 
                        let infO = Some (inf.Trim().ToLower())
                        es.aliados 
                        |> Seq.tryFind (fun al -> al.influyente = infO )
                        |> ResultM.ofOption (fun () -> ResultMessage.ErrorMsg <| sprintf "Influyente no encontrado : %s" inf)
                |>  absorbR
        
            let obtenerIdInfluyente : AA<_, _> =
                obtenerInfluyente
                >-> fun al -> al.id.Id
        ...
        //                })
            ...
            let obtenerAliadoEstadoPerfilSubIds =
                ObtenerEstado.obtenerAliadoEstadoId
                >=>(fst
                >>> buscarPerfil
                >|> mapBoth (fun (al,md) (pe,subs) -> al, md, pe, subs ) )
        
            let crearSubscripcion claims =
                fun _ -> obtenerAliadoEstadoPerfilSubIds claims 
                >|> bindBoth (fun (IdPayment idp) (al : Aliado, md : Modelo, pe, subs)  -> asyncResultM {
            ...
            let createSubscription : AA<_,_> =
                fun sub -> ARBCreateSubscriptionRequest(subscription = sub)
                >>> ARBCreateSubscriptionController
                @-> fun response -> response.subscriptionId
        
            let updateSubscription subId : AA<_,_> =
                fun sub -> ARBUpdateSubscriptionRequest(subscription = sub, subscriptionId = subId)
                >>> ARBUpdateSubscriptionController
                @-> fun _response -> subId
        
            let actualizarSubscripcion claims =
                crearSubscripcion claims
                >=> function
                    | sub, None       -> createSubscription       sub
                    | sub, Some subId -> updateSubscription subId sub
        
        //    let getBatchDetalles :  AA<_,_> =
        //        fun bid -> getTransactionListRequest(  batchId = bid )
        //        >>> getTransactionListController
        //        @-> fun response -> response.transactions
        //
        //    let buscarPerfilPorProfileId :  AA<_,_> =
        //        fun pid -> getCustomerProfileRequest(  customerProfileId = string pid )
        //        >>> getCustomerProfileController
        //        @-> fun response -> response.profile
        //
        //    let getSubscriptions :  AA<_,_> =
        //        fun () -> ARBGetSubscriptionListRequest(searchType = ARBGetSubscriptionListSearchTypeEnum.subscriptionActive)
        //        >>> ARBGetSubscriptionListController
        //        @-> fun response -> response.subscriptionDetails
        //        >=> tee (fun ss -> asyncResultM { do! sprintf "Subscriptions %d" ss.Length |> Info })
        //        >=> AsyncResultM.traverseSeq (fun s -> buscarPerfilPorProfileId s.customerProfileId |> map (fun p -> s, p) )
        //
        //    let getSubscriptionTransactions :  AA<_,_> =
        //        fun () -> getSettledBatchListRequest(  firstSettlementDate = System.DateTime.Today.AddMonths -1 
        //                                            ,   lastSettlementDate = System.DateTime.Now)
        //        >>> getSettledBatchListController
        //        @-> fun response -> response.batchList
        //        >=> AsyncResultM.traverseSeq (fun b -> getBatchDetalles b.batchId |> map (fun ts -> b, ts) )
        //        >->(Seq.map   (fun (b,ts) -> b, ts |> Seq.filter(fun t -> t.subscription <> null) )
        //        >>> Seq.filter(fun (b,ts) ->    ts |> Seq.isEmpty |> not )
        //           )
        //
        //    let SubsYTransacciones : AA<_,_> =
        //        getSubscriptionTransactions
        //        >-> Seq.collect (fun (b, ts) -> ts |> Seq.map (fun t -> b, t))
        //        >|> pairWith ()
        //        >=> bindSnd  getSubscriptions
        //        >=> tee (fun (ts, subs) -> asyncResultM {
        //                let orphan = ts |> Seq.filter(fun (b,t) -> subs |> Seq.exists (fun (s,p) -> s.id = t.subscription.id) |> not)
        //                for b, t in orphan do
        //                    do! sprintf "Huerfano: %A" (
        //                                b.batchId
        //                                , t.accountNumber
        //                                , t.accountType
        //                                , t.firstName
        ...
        module Rpc =
            ...
            let serverEndPoint = 
                lazy 
                    ...
                    else     ambiente.VariableAmbiente "Website" |> sprintf "https://%s/api/"
            ...
            let invoke (fname:string) idAliado (p:string) = asyncResultM {
                ...
                let  req  = RequestOptions( 
                                ...
                                , Headers     = headers
                                , Body        = p)
                let! resp = JS.Fetch(serverEndPoint.Value + fname, req ) |> Promise.AsAsync
        ...
        //
            ...
            let [< Rpc >] obtenerIdInfluyente       codigo       : AR<_> = ManejadorEventos.obtenerIdInfluyente      codigo
            let [< Rpc >] actualizarSubscripcion (id:IdAliado) (idp: IdPayment)  : AR<_> = Authorize.actualizarSubscripcion id idp
        ...
        module AmbienteMemoria =
            ...
            let setAmbiente() =
                ambiente <- {
                    ...
                    member __.GuardarTransaccion ida tra   = AsyncResultM.errorMsgf "Ambiente.GuardarTransaccion : not implemented"
                }
