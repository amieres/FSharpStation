#nowarn "3242"
#nowarn "42"
////-d:DLL -d:FSharpStation1614439460864 -d:WEBSHARPER
...
//#define FSharpStation1614439460864
#if !NOFSROOT
#if INTERACTIVE
module FsRoot   =
#else
    ...
    open WebSharper
        ...
        module LibraryJS =
                ...
                let calcUI wrapper allowDelete (lmd:AF.ListModelData<_,Snippet>) =
                    ...
                    fun (k:View<_ option>) (snp:Var<Snippet>) ->
                        ...
                        Templates.Snippet()
                            ...
                            .TogglePred(    fun _ ->
                                            let preds = Lens lmd.CurrentV.V.snpPredIds
                                            preds.Value
                                            |> (if Set.contains snp.Value.snpId preds.Value then Set.remove else Set.add) snp.Value.snpId
                                            |> preds.Set
                            )                                                                
            ...
            //
                ...
                let predecessorsW = View.Do {
                    let! snp   = currentSnippetV.View
                    let! snps  = snippetsLM.View
                    let  preds = allPredecessors snp |> Set.map (fun snp -> snp.snpId)
                    return 
                        snps
                        |> Seq.filter (fun snp  -> Set.contains snp.snpId preds)
                }
            
                let getPresencesW = View.Do {
                    let! preds = predecessorsW
                    let  ids   = 
                        preds
                        |> Seq.map    (fun snp -> snp.snpId)
                        |> Seq.map    (sprintf "%A" )
                    return
                        if Seq.isEmpty ids then "" else
                        ids
                        |> String.concat "; "
                        |> sprintf "CodePresence.get([| %s |])"
                }
            
                let presencesV = Var.Create ""
                ...
                let codeRequiredW = View.Do {
                    let! pairs     = presencesV.View
                    let  presences =
                        pairs.Split '\n'
                        |> Seq.map    String.trim
                        |> Seq.choose(String.splitInTwoO " ")
                        |> Seq.map   (fun (a, b) -> ParseO.parseGuidO  a |> Option.map UoM.Tag
                                                  , ParseO.parseInt64O b |> Option.map long2Date)
                        |> Seq.choose(function (Some a) , Some b -> Some (a, b) |_-> None)
                        |> Map
                    let! preds = predecessorsW
                    let  reqs  =
                                preds
                                |> Seq.skipWhile(fun snp -> 
                                    match Map.tryFind snp.snpId presences with
                                    | Some m when m = snp.snpModified -> true
                                    |_-> false
                                )
                    let res = reqs |> Seq.map (fun snp -> sprintf "#line 1 @%A\n%s\nCodePresence.add %A \"%d\"" snp.snpName snp.snpContent snp.snpId (date2Long snp.snpModified ) )
                                |> (if pairs = "--" then Seq.append [ installPresence ] else id )
                                |> String.concat "\n\n"
                    //if res = "" then return res else
                    //let! curSnpNm = currentSnippetNameV.View
                    return res
                }
                ...
                let plugInAdded =
                    AF.plugin {
                        ...
                        plgView   "getPresences"   getPresencesW
                        plgVar    "presences"      presencesV
                        plgView   "codeRequired"   codeRequiredW
