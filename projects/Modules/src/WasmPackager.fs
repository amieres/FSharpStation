...
namespace FsRoot
    ...
    open WebSharper
        ...
        module LibraryNoJS =
            ...
            module FsCompiler =
                ...
                let fSharpError2ResultMessage (error : FSharpErrorInfo) =
                    let indent, file = 0, System.IO.Path.GetFileNameWithoutExtension error.FileName
                ...
                let compileRm() = fusion {
                    ...
                    let! msgs, exit = args
                                      ...
                                      |> ofAsync
                    do!               msgs 
                                      |> Seq.map fSharpError2ResultMessage 
                                      |> ResultMessage.reduceMsgs 
                                      |> ofResultMessage
                    if exit <> 0 then do! ErrorF NoMsg
                }         
                let compileMsgs args = compileRm() |> runReader args |> Async.RunSynchronously |> (Result.map snd >> Result.defaultWith id)
                let compile     args = compileMsgs args |> ResultMessage.summarized |> printfn "%s"
        ...
        module Library2 =
            ...
            module WebSockets =
                ...
                #else
                    "http://localhost:9005/#/Snippet/3c04a2c0-cdcd-4899-a9fd-f5033f11ccb5"
                #endif
    ...
    module FsCompileDll =
        ...
        let compileSnippetRm         (show:bool) snpName = fusion {
            ...
            let! info = prepareCodeFromSnippet   snpName
            do!         FsCompiler.compileRm()
            return info
        }
        ...
        let compileSnippet                         show snpName = 
            fusion {
                ...
                let! _ = compileSnippetRm          show snpName
                return sprintf "Finished compiling %s!" snpName
            }
            |>  mapReader (CommArgCollection [])
            |>  FusionAsyncM.iterResultPrintA
    ...
    module WasmPackager =
        ...
        let stringArg arg v = sprintf "--%s=\"%s\"" arg v
        ...
        let wpkPackagerDir    = CommArg.New("WasmSdk"        , true , sprintf "%s/packager.exe"            ) // location of packager.exe
    
        let wpkDll            = CommArg.New("Dll"            , false, sprintf   "%s"                       ) // Include foo.dll as one of the root assemblies
        let wpkSearchPath     = CommArg.New("search-path"    , false, stringArg "search-path"              ) // search-path=x Add specified path 'x' to list of paths used to resolve assemblies
        let wpkNativeLib      = CommArg.New("native-lib"     , false, stringArg "native-lib"               ) // Link the native library 'x' into the final executable.
        let wpkPreloadFile    = CommArg.New("preload-file"   , false, stringArg "preload-file"             ) // Preloads the file or directory 'x' into the virtual filesystem.
        let wpkEmbedFile      = CommArg.New("embed-file"     , false, stringArg "embed-file"               ) // Embeds the file or directory 'x' into the virtual filesystem.    
    
        let wpkHelp           = CommArg.New("help"           , true , flagArg   "help"                     ) // Show command arguments
        let wpkExecMode       = CommArg.New("mono-sdkdir"    , true , argExecMode                          ) //  
        ...
        let wpkOut            = CommArg.New("out"            , true , stringArg "out"                      ) // out           Set the output directory to 'x' (default to the current directory )
        let wpkMonoSdkdir     = CommArg.New("mono-sdkdir"    , true , stringArg "mono-sdkdir"              ) // mono-sdkdir=x Set the mono sdk directory to 'x'
        let wpkDeploy         = CommArg.New("deploy"         , true , stringArg "deploy"                   ) // deploy        Set the deploy prefix to 'x' (default to 'managed'  )
        let wpkVfs            = CommArg.New("vfs"            , true , stringArg "vfs"                      ) // vfs           Set the VFS prefix to 'x' (default to 'managed'  )
        let wpkTemplate       = CommArg.New("template"       , true , stringArg "template"                 ) // template      Set the template name to  'x' (default to 'runtime.js'       )
        let wpkAsset          = CommArg.New("asset"          , true , stringArg "asset"                    ) // asset         Add specified asset 'x' to list of assets to be copied
        ...
        let invokePackager() = fusion {
            let! exec  = buildArgRm wpkPackagerDir |> Fusion.ofFusionM
            let! ops   = argumentsRm true isWpk    |> Fusion.ofFusionM
            return ops
                |> String.concat "  "
                |> RunProcess.runToFinish exec
        }
    
        let compileAndPackage show snpName = fusion {
            let! info  = FsCompileDll.compileSnippetRm show snpName
            let  dirs  = info.assemblies |> Seq.map System.IO.Path.GetDirectoryName |> Seq.append info.prepIs |> Seq.distinct
