#nowarn "3242"
////-d:DLL -d:FSharpStation1614641116564 -d:TEE -d:WEBSHARPER -d:WEBSHARPER47
...
namespace FsRoot
    ...
    open WebSharper
        ...
        module LibraryJS =
            ...
            //#r "D:\Abe\CIPHERWorkspace\FSharpStation/projects/LayoutEngine/bin/LayoutEngine47.dll"
            #else
            //#r "D:\Abe\CIPHERWorkspace\FSharpStation/projects/LayoutEngine/bin/LayoutEngine45.dll"
            #endif
            //#define DLL
            module MonacoPlugIn =
                ...
                let [< Inline >] inline tryDeserialize def json = try Json.Deserialize json with e-> def
                let [< Inline >] inline decoded def sA = sA |> Async.map (tryDeserialize def)
            
                let createMonacoEditor0 txtV langW themeW annotW (cmdResp: (Var<string> * View<string>) option) =
                    let annotW' = annotW |> View.Map (tryDeserialize [||] >> Seq.ofArray)
                    let editor =
                        match cmdResp with
                        | Some (cmdV, respW) -> 
                            let mutable okkoca = None
                            let receiveResponses (resp:string) =
                                match okkoca with
                                | None -> ()
                                | Some (ok, ko, ca) -> 
                                okkoca <- None
                                ok resp
                            let sendCommandGetResponse cmd (p:GenEditor.Position) = 
                                async {
                                    match okkoca with Some(_,_,ca) -> ca(System.OperationCanceledException()) |_-> ()
                                    sprintf "%s %d %d" cmd p.line p.col |> cmdV.Set
                                    return! Async.FromContinuations(fun fns -> okkoca <- Some fns )
                                }
                            respW |> View.Sink receiveResponses            
                            { monacoNew txtV langW themeW 
                                with    annotations = annotW' 
                                        toolTip        = Some(fun _ p -> sendCommandGetResponse "tooltip"        p |> decoded ""   |> Async.map Some) 
                                        declaration    = Some(fun _ p -> sendCommandGetResponse "declaration"    p |> decoded None )
                                        autoCompletion = Some(fun _ p -> sendCommandGetResponse "autocompletion" p |> decoded [||] )
                            }
                        | None -> { monacoNew txtV langW themeW with annotations = annotW' }
                    Html.div [] [ editor |> GenEditor.generateDoc ]
            
                let createMonacoEditor4 =
                    AF.depWithExtracts     (fun (extractAts, extractDoc, extractText) txtv (lang:string) (theme:string) (annotations:string) ->
                        AF.docWithVar      (fun txtV     ->
                                createMonacoEditor0 txtV (extractText lang) (extractText theme) (extractText annotations) None
                            ) txtv
                    )
            
                let createMonacoEditor5 =
                    AF.depWithExtracts     (fun (extractAts, extractDoc, extractText) txtv (lang:string) (annotations:string) (cmdv) (response:string) ->
                        AF.docWithVar      (fun txtV ->
                            AF.docWithVar  (fun cmdV ->
                                createMonacoEditor0 txtV (extractText lang) (View.Const "")  (extractText annotations) (Some (cmdV, (extractText response)) )
                            ) cmdv
                            ) txtv
                    )
                ...
                let plugInAdded =
                    AF.plugin {
                        plgName   "Monaco"
                        plgDoc4   "editor"    createMonacoEditor4 "var" "Language" "theme" "Annotations"
                        plgDoc5   "editor5"   createMonacoEditor5 "var" "Language"         "Annotations" "cmdV" "respW"
                    } 
                    |> AF.addPlugIn
