...
#nowarn "1182"
#nowarn "1178"
////-d:FSharpStation1560769401929 -d:NETSTANDARD20 -d:NOFRAMEWORK --noframework -d:TEE -d:WEBSHARPER
...
//#define FSharpStation1560769401929
#if INTERACTIVE
module FsRoot   =
#else
namespace FsRoot
    ...
    module ProzperServer =
            ...
            | Cambiado
                with 
                    static member Parse (s:string) = 
                        ...
                        | "Expirado"                        -> Expirado 
                        | "Cancelado"                       -> Cancelado
                        | String.StartsWith "Verificado " r -> Verificado r
                        | String.StartsWith "Rechazado "  r -> Rechazado r
                        | _                                 -> Subido
                    override this.ToString() = sprintf "%A" this
        ...
        //        >=> obtenerFormasDePagoId
            ...
            let obtenerTransaccionesPara aid = eff {
                let! modelo   = ObtenerEstado.obtenerEstadoParaUsuario aid
        ...
        module Rpc =
            ...
            let [< Rpc >] obtenerEstadoParaUsuario aid           : AR<_> = ObtenerEstado.obtenerEstadoParaUsuario aid            |> AzureHandler.run// |> Async.rtn
            ...
            let [< Rpc >] obtenerTransaccionesPara        aid    : AR<_> = Authorize.obtenerTransaccionesPara                aid    |> AzureHandler.run// |> Async.rtn
            let [< Rpc >] refrescarTransacciones              () : AR<_> = reader.RefrescarTransacciones                      ()    |> AzureHandler.run// |> Async.rtn
        ...
        module ClientSide =
            ...
            //                                            ,  storeAuthStateInCookie    = false ))
                ...
                let getTokenO0() = async {
                    ...
                    | Some user  -> try 
                                    ...
                                    with e ->
                                        let! auth = agent.Value.acquireTokenPopup( authParms () ) |> Promise.AsAsync 
                                        return Some auth.accessToken
                    | None       -> return None
                    //return
            ...
            module ModeloUI =
                ...
                module Rpc =
                    ...
                    let obtenerListaDocs         p = llamar            Rpc.obtenerListaDocs                            p
                    ...
                    //let obtenerTransaccionesPara p = llamar            Rpc.obtenerTransaccionesPara   Aliado.LoggedId
                    let refrescarTransacciones   p = llamar            Rpc.refrescarTransacciones                      p
                    ...
                    let formaDoc (tarV  : Var<(TarjetaCredito * string * bool)>) =
                        ...
                        let forma =
                            TemplateLib.TarjetaCredito()
                                ...
                                .Disabled4(    attr.disabledDynPred (View.Const "") disabledW       )
                                .Disabled5(    attr.disabledDynPred (View.Const "") disabledW       )
                                .Visible(      if disabledW.V then "display:None" else ""           )
                                .Create()
                ...
                module FormaFirma =
                    ...
                    let subirScreenShot tipo filenamePre = async {
                        ...
                        locationV.Set loc
                        do! Async.Sleep 100
                        let! canvas = html2canvas(JS.Document.QuerySelector("#content-wrapper")) |> Promise.AsAsync
                        JS.Document.Body.AppendChild canvas |> ignore
                        let filename = sprintf "%s %s.png" filenamePre <| fechaHoraV.Value.Replace(":",".")
                        canvas?toBlob (fun (blob:Blob) ->
                            let fd = FormData()
                        ...
                        let! res = ActualizarArchivos(Aliado.LoggedId, [| {
                    ...
                    let formaFirma (nombre:View<string>) (contenido:Doc) tipo procesar filenamePre =
                        let firmarDocumento() = 
                            asyncResult {
                                ...
                                return! procesar() |> AsyncResult.map JS.Alert
                            } |> Rpc.iterA
                    ...
                    let formaAutorizacion () =
                        let formaContenido  =
                            TemplateLib.AutorizacionPago()
                                ...
                                .Doc()
                        FormaFirma.formaFirma titular.View formaContenido Autorizacion actualizarSubscripcion "AutorizacionPago"
                ...
                module Pendientes =
                    ...
                    let agregarCuenta () =
                        [ 
                            ...
                            Html.a [ attr.href "/#/Content/ProzperLyt.cntFormaFormasPago" ] [ Html.text "Tarjeta de Credito o Cuenta Bancaria" ]
                        ] |> Seq.ofList
                
                    let firmarContrato () =
                        [ 
                            Html.text "Firmar "
                            Html.a [ attr.href "/#/Content/ProzperLyt.cntFormaContrato" ] [ Html.text "Contrato" ]
                        ] |> Seq.ofList
                
                    let firmarAutorizacion () =
                        [ 
                            Html.text "Firmar "
                            Html.a [ attr.href "/#/Content/ProzperLyt.cntFormaFormasPago" ] [ Html.text "Autorizacion (de click en: AUTORIZAR DEBITO)" ]
                        ] |> Seq.ofList
                
                    let subirArchivoTipo (tipo:TipoArchivo) =
                        [ 
                            Html.text "Subir "
                            Html.a [ attr.href "/#/Content/ProzperLyt.cntSubirDocumentos" ] [ Html.text (sprintf "%A" tipo) ]
                        ] |> Seq.ofList
                
                    let subirIdentificacion () =
                        [ 
                            Html.text "Subir documento de identidad: "
                            Html.a [ attr.href "/#/Content/ProzperLyt.cntSubirDocumentos" ] [ Html.text "Cedula, Pasaporte, Licencia de Conducir" ]
                        ] |> Seq.ofList
                
                    let pendienteArchivo (ar:Archivo) =
                        [ 
                            Html.a [ attr.href (sprintf "https://prozper0.azurewebsites.net/documentos/%s/%s" ar.idAliado.Id ar.nombre) ] [ Html.text (sprintf "%A" ar.tipo) ]
                            Html.text " pendiente de Verificacion"
                        ] 
                
                    let verificarArchivo (ar:Archivo) =
                        match ar.status with
                        | Verificado s -> [ Html.text (sprintf "%A Verificado (%s)" ar.tipo s) ] 
                    ...
                    let chequearArchivos es (subirlo:unit-> Doc seq) archivos =
                        ...
                        | docs -> docs      |> Seq.map verificarArchivo
                
                    let verificarCuenta (fp:FormaPago) =
                        match fp.authorizeIdR with 
                        | Ok id   -> [ Html.text (sprintf "%s Registrada (%s)"                 (fp.cuentaPago.ToString()) id.Id) ]
                        | Error m -> [ Html.text (sprintf "%s NO REGISTRADA en Authorize (%s)" (fp.cuentaPago.ToString()) m    ) ]
                        |> Seq.ofList
                ...
                module ReporteConsolidado =
                    ...
                    let consolidado() =
                        ...
                        let aliado2Fila al = {
                            archivos      =      if al.docPendientes = 0 then sprintf "%2d" al.documentos else sprintf "(%2d)%2d" al.docPendientes al.documentos
                            nombre        =      Aliado.nombre       al.datosPersonales 
                            correo        =      Aliado.correo       al
                        ...
                        let ordenV = 
                            Var.Create {
                                archivos      =   1, false, (fun (d:Fila<string>) -> d.archivos     .ToUpper() )
                                nombre        =  14, true , (fun (d:Fila<string>) -> d.nombre       .ToUpper() )
                                correo        =  13, true , (fun (d:Fila<string>) -> d.correo       .ToUpper() )
                                ...
                                descendientes =   2, false, (fun (d:Fila<string>) -> d.descendientes.ToUpper() )
                        ...
                        let toggle (v:int * bool * (Fila<string> -> string) ) = 
                            {
                                archivos      = newOrden v ordenV.Value.archivos
                                nombre        = newOrden v ordenV.Value.nombre       
                                correo        = newOrden v ordenV.Value.correo
                        ...
                        let listaArchivos = lazy Html.tr [ attr.bgcolor "beige" ] [ Html.td [ attr.colspan "12" ] [ ListaDocs.listaDocs() ] ]
                        ...
                        TemplateLib.Consolidado()
                            ...
                            .sort_archivos(      fun _ -> toggle ordenV.Value.archivos     )
                            .sort_nombre(        fun _ -> toggle ordenV.Value.nombre       )
                            .sort_correo(        fun _ -> toggle ordenV.Value.correo       )
                            ...
                            .TBody(
                                ...
                                |> Doc.BindSeqCachedViewBy (fun al -> al.Id) (fun alid fW ->
                                    TemplateLib.FilaConsolidado() 
                                        ...
                                        .expandFiles(   fun _ -> (if ListaDocs.expandedOV.Value = Some (IdAliado alid) then None else Some (IdAliado alid)) |> ListaDocs.expandedOV.Set)
                                        .expanded(      fW.V.archivos)
                                        .Doc()
                                )
                            ).Doc()
                module TablaPagos =
